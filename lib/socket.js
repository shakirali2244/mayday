          ...
          if(data.message == ''){
           ...
           if (msg.indexOf('/') == -0){
            ...
            if (msg.indexOf('giphy') >-1){
              writeTofile(data,Object.keys(socket.rooms)[1]);
              socket.broadcast.to(Object.keys(socket.rooms)[1]).emit('chatMessage',data );
              var limit = 10;
              var query = '?q=';
              ...
              http.get({
                host: 'api.giphy.com',
                path: '/v1/gifs/search'+query+'&limit='+limit+'&api_key=dVfeE6CWksEHyoRikB652kaCWXPi7E9y&offset=0&rating=G&lang=en'
            }, function(response) {
                ...
                response.on('end', function() {
                  ...
                  if(Object.keys(socket.rooms).length>1 && url != undefined){
                    console.log('giphybot : ' + url + ' in ' + socket.rooms);
                    var tempMsgObj = {name: 'giphybot' ,message: url}
                    writeTofile(tempMsgObj,Object.keys(socket.rooms)[1]);
                    io.to(Object.keys(socket.rooms)[1]).emit('chatMessage',tempMsgObj );
                  }
                });
            });
            }
           }else{
            var currentRoom = roomInfo[Object.keys(socket.rooms)[1]];
             if(currentRoom != undefined){
               var found = false;
               for (var i=0; i<currentRoom.length; i++) {
              ...
              if(!found){
                ...
                writeTofile(data,Object.keys(socket.rooms)[1]);
               socket.broadcast.to(Object.keys(socket.rooms)[1]).emit('chatMessage', data);
               //console.log(roomInfo[Object.keys(socket.rooms)[1]]);
             }
          }
        }
